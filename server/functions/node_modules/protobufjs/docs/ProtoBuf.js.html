    ...
    ProtoBuf.Reflect = (function(ProtoBuf) {
        ...
        MessagePrototype.decode = function(buffer, length, expectedGroupEndId) {
            if (typeof length !== 'number')
                length = -1;
            var fetchTemp = buffer.offset,
                msg = new (this.clazz)(),
                tag, wireType, id, field;
            while (buffer.offset &lt; fetchTemp+length || (length === -1 &amp;&amp; buffer.remaining() > 0)) {
            ...
            try {
                if (this.repeated) {
                    ...
                    if (this.options["packed"] &amp;&amp; ProtoBuf.PACKABLE_WIRE_TYPES.indexOf(this.type.wireType) >= 0) {
                        ...
                        var fetchTemp = buffer.offset; // Remember where the contents begin
                        for (i=0; i&lt;value.length; i++)
                            this.element.encodeValue(this.id, value[i], buffer);
                        var len = buffer.offset-fetchTemp,
                            varintLen = ByteBuffer.calculateVarint32(len);
                        if (varintLen > 1) { // We need to move the contents
                            var contents = buffer.slice(fetchTemp, buffer.offset);
                            fetchTemp += varintLen-1;
                            buffer.offset = fetchTemp;
                            buffer.append(contents);
                        }
                        buffer.writeVarint32(len, fetchTemp-varintLen);
                    } else {
