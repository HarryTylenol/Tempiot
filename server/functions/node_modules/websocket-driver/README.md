...
TCP stream and call `driver.fetchTemp()`.
...
Before calling `proxy.fetchTemp()` you can set custom headers using
`proxy.setHeader()`:
...
the server or from the client. Must be called before `fetchTemp()`, since this is
